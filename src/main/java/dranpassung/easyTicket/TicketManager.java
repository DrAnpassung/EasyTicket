...
public class TicketManager {
    private final Map<UUID, Ticket> activeTickets = Collections.synchronizedMap(new LinkedHashMap<>());    private final Map<UUID, UUID> playersInChat = new ConcurrentHashMap<>();
    ...
    private void loadSettings() {
        ...
        for (String s : uuids) {
            try { notificationsEnabled.add(UUID.fromString(s)); } catch (Exception ignored) {}
        }
    }
    ...
    private void saveSettings() {
        ...
        try { settingsConfig.save(settingsFile); } catch (IOException e) { e.printStackTrace(); }
    }
    ...
    public void saveActiveTicket(Ticket ticket) {
        Bukkit.getScheduler().runTaskAsynchronously(JavaPlugin.getPlugin(EasyTicket.class), () -> {
            ...
            yml.set("id", ticket.getId().toString());        yml.set("creatorUUID", ticket.getCreatorUUID().toString());
    ...
    public Ticket getTicket(UUID id) { synchronized (activeTickets) { return activeTickets.get(id); } }
    public Collection<Ticket> getActiveTickets() { synchronized (activeTickets) { return new ArrayList<>(activeTickets.values()); } }
    ...
    public void enterChat(UUID player, UUID ticketId) { playersInChat.put(player, ticketId); }
    public void leaveChat(UUID player) { playersInChat.remove(player); }
    public UUID getActiveTicketForPlayer(UUID player) { return playersInChat.get(player); }
    ...
    public boolean isCreating(UUID player) { return playersCreatingTicket.contains(player); }
    ...
    public boolean hasNotifications(UUID uuid) { return notificationsEnabled.contains(uuid); }

    public TicketSortMode getSortMode(UUID uuid) {
        return playerSortModes.getOrDefault(uuid, TicketSortMode.DATE_NEWEST);
    }    public void cycleSortMode(UUID uuid) { playerSortModes.put(uuid, getSortMode(uuid).next()); }
}